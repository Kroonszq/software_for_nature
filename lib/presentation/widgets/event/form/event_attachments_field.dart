...
class EventAttachmentsField extends StatelessWidget {
  ...
  Future<void> _pickFiles(EventFormBloc bloc) async {
    ...
    final attachments = result.files.map((f) => EventAttachment(name: f.name, path: f.path, size: f.size)).toList();
    if (attachments.isNotEmpty){
      bloc.add(AttachmentsAdded(attachments));
    }
  }
          ...
          children: [
            ...
            ...state.attachments.map(
              (a) => ListTile(
                ...
                subtitle: a.size != null ? Text(TimeUtils.formatSize(a.size ?? 0)) : null,
                trailing: IconButton(
